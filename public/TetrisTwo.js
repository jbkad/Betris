...
document.addEventListener('keydown', async(e)=>{
    if(e.key=='Enter' && !gameStarted){
        ...
        for(let i=0; i<rotateData.length; i++){
            if(activeBlockType==rotateData[i].type){
...
function overlapTest(array){
    let data = array.map(square => `${marginLeft(square)} ${marginTop(square)}`)
...
function getLineValues(){
    let lineValues = []
    let counArray = new Array(20).fill(0)
    document.querySelectorAll('#play-board img').forEach(square => counArray[marginTop(square)/26] += 1)
    counArray.forEach((count,i)=>{
        if(count==10){
